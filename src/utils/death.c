...
/*   Updated: 2024/05/15 09:54:48 by fschuber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pthread_mutex_lock(philo_inputs->inputs.death_flag_mutex);
	if (philo_inputs->inputs.death_flag == 0)
	{
		philo_inputs->inputs.death_flag = 1;
		log_philo_action(philo_inputs, "died", "💀");
		pthread_mutex_unlock(philo_inputs->inputs.death_flag_mutex);
		return ;
	}
	pthread_mutex_unlock(philo_inputs->inputs.death_flag_mutex);
	return ;
